...
<script lang="ts">
  ...
  import { syncWorker, syncWrap } from '$lib/workers/worker_legacy';
  import type { WorkerType } from '$lib/workers/sync_worker_legacy';
  import type { Query, Filter, SearchWithSeed, ReverseSearchConfig, SearchResults, StatConfig } from '$lib/skill_tree_legacy';

  import SearchResultsComponent from '$lib/components/Legacy/SearchResults.svelte';
  import { statValues } from '$lib/values';
  import { data, calculator } from '$lib/types/LegacyTypes.js';

  import TradeButton from '$lib/components/Legacy/TradeButton.svelte';
...
<div class="min-h-screen bg-gray-900 text-white">
  <div class="container mx-auto px-4 py-8">
    ...
    <!-- Jewel Selection -->
    <div class="mb-4">
      <label for="timeless-jewel-select" class="block text-sm font-medium mb-2">Timeless Jewel</label>
      <Select inputId="timeless-jewel-select" items={jewels} bind:value={selectedJewel} on:change={updateUrl} />
    </div>

    <!-- Conqueror Selection -->
    {#if selectedJewel}
      <div class="mb-4">
        <label for="conqueror-select" class="block text-sm font-medium mb-2">Conqueror</label>
        <Select inputId="conqueror-select" bind:value={dropdownConqueror} on:change={updateUrl} />
      </div>
    {/if}
    ...
    <div class="mb-4">
      ...
      <input id="seed-input" type="number" bind:value={seed} on:input={updateUrl} class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded text-white" />
    </div>
