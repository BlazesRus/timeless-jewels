...
export class ModernPerformanceMonitor {
  ...
  public async measureAsync<T>(operationName: string, asyncFn: (signal: AbortSignal) => Promise<T>): Promise<T> {
  ...
  public createLazyLoadObserver(callback: (entries: IntersectionObserverEntry[]) => void, options?: IntersectionObserverInit): IntersectionObserver {
