...
<script lang="ts">
  ...
  import { addDebugMessage } from '$lib/ModernWasm/debugLogger.svelte';
  ...
  $effect(() => {
    if (autoOptimize && intersection.isVisible()) {
      ...
      const optimizationTasks = [() => addDebugMessage('Optimizing rendering...'), () => addDebugMessage('Pre-loading resources...'), () => addDebugMessage('Cleaning up unused data...')];
