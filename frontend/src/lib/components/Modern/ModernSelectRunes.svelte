<!-- 
Modern Select Component for Svelte 5 with Runes (Client-Only)
Full-featured replacement for svelte-select using native Svelte 5 runes and advanced patterns
This component should only be loaded on the client to avoid SSR issues with runes.
 *
...
<div bind:this={containerRef} class="modern-select relative themed">
  ...
  {#if isOpen && !disabled}
    <div bind:this={dropdownRef} class="select-dropdown" role="listbox">
      ...
      <div class="options-container">
        {#each filteredItems as item, index}
          <button class="select-option" class:selected={value?.value === item.value} class:highlighted={highlightedIndex === index} onclick={() => handleSelect(item)} type="button" role="option" aria-selected={value?.value === item.value} data-index={index}>
            {item.label}
          </button>
        {/each}
