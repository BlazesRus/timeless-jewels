...
<script lang="ts">
  ...
  const filteredResults = $derived.by(() => {
    ...
    return searchResults.filter(result => result.name?.toLowerCase().includes(searchQuery.toLowerCase()));
  });
  ...
  let { data = $bindable([]), onSelect, placeholder = 'Search...', maxResults = 50, children }: Props = $props();
...
<div bind:this={containerElement} class="modern-search-container" style="container-type: inline-size;" role="search" aria-label="Search interface">
  <div class="search-input-wrapper">
    <input bind:value={searchQuery} onkeydown={handleKeydown} type="search" placeholder={placeholder} class="modern-search-input" aria-label="Search input" aria-controls="search-results" autocomplete="off" />
  ...
  {#if hasResults && isVisible}
    <ul id="search-results" class="search-results" role="listbox" aria-label="Search results">
      {#each filteredResults as result, index (result.id || index)}
        <li class="search-result-item" role="option" aria-selected="false" tabindex="0" onclick={() => handleSelect(result)} onkeydown={e => e.key === 'Enter' && handleSelect(result)}>
          {#if children}
