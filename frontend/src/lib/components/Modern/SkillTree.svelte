...
<script lang="ts">
  ...
  const drawSprite = (context: CanvasRenderingContext2D, path: string, pos: Point, active = false, mirrored = false) => {
    ...
    if (!sprite) {
      return; // Early return if sprite not found
    }
    ...
    if (!self) {
      return; // Early return if coordinates not found
    }
    ...
    const image = (active ? spriteCacheActive : spriteCache)[spriteSheetUrl];
    if (!image) {
      return; // Early return if image not loaded
    }

    context.drawImage(image, self.x, self.y, self.w, self.h, topLeftX, finalY, newWidth, newHeight);

    if (mirrored) {
      context.save();
      context.translate(topLeftX, topLeftY);
      context.rotate(Math.PI);
      context.drawImage(image, self.x, self.y, self.w, self.h, -newWidth, -(newHeight / 2), newWidth, -newHeight);
      context.restore();
    }
  };
  ...
  const render: RenderFunc = $derived((({ context, width, height }) => {
    ...
    Object.keys(drawnNodes).forEach((nodeId: string) => {
      ...
      if (!node || !node.orbit || node.orbitIndex === undefined) return;
      ...
      node.out?.forEach((o: any) => {
        ...
        if (node.group != targetNode.group || node.orbit != targetNode.orbit) {
          ...
          if (node.group && node.orbit !== undefined) {
            ...
            if (group && node.orbit !== undefined && skillTree.constants?.orbitRadii?.[node.orbit]) {
              const groupPos = toCanvasCoords(group.x, group.y, offsetX, offsetY, scaling);
              const orbitRadius = skillTree.constants.orbitRadii[node.orbit];
              if (orbitRadius !== undefined) {
                context.arc(groupPos.x, groupPos.y, orbitRadius / scaling + 1, finalA, finalB);
              }
            }
          }
        }
    ...
    Object.keys(drawnNodes).forEach((nodeId: string) => {
      ...
      if (distance(rotatedPos, mousePos) < touchDistance / scaling) {
        ...
        //addDebugMessage('Hovering over node:', node.skill, node.name, 'at distance:', distance(rotatedPos, mousePos), 'touchDistance:', touchDistance / scaling);
      }
    });
