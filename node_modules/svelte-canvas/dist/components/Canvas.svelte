...
let { width: _width, height: _height, pixelRatio: _pixelRatio, class: className, style = '', autoplay = false, autoclear = true, layerEvents = false, onresize, contextSettings, children, ...handlers } = $props();
let canvas;
let context;
let layerRef;
...
const pixelRatio = $derived.by(() => {
    ...
    if (_pixelRatio && _pixelRatio !== 'auto')
        return _pixelRatio;
